---
title: 'Spatial Transcriptomic Analysis'
date: '`r Sys.Date()`'
output:
  html_document:
    toc: true
...
Load necessary libraries for spatial transcriptomic analysis, including Seurat for single-cell data handling and visualization tools.
...
Load the Xenium dataset for the Multiple Sclerosis (MS) sample.

```{r}
raw_data_dir <- "path/to/raw_data" # Define your raw data directory
setwd(raw_data_dir)
MS_sample <- "sample_name" # Define your sample file name
sc <- LoadXenium(MS_sample, fov = "fov")
sc$orig.ident <- "dura"
sc$sample_ID <- "MS_sample"
Idents(sc) <- "dura"

result_dir <- "path/to/results" # Define your result directory
setwd(result_dir)
...
Evaluate the total number of UMIs (counts) per bin (cell) to assess overall transcript abundance. Bins with extremely low or high counts are filtered out to remove low-quality cells or potential doublets.
...
Assess the number of detected genes per bin. Similar to counts, extreme values are filtered.
...
Apply the filtering criteria defined in the QC steps.

#### 1.2.3.1 Features (and Counts)

Filter cells based on feature counts.
...
Remove genes that are expressed in very few bins to reduce noise.
...
Standard pre-processing steps including normalization, variable feature identification, scaling, and dimensionality reduction.
...
Identify features that exhibit high cell-to-cell variation.
...
Scale the data to ensure that highly expressed genes do not dominate the analysis.
...
Subset a specific region of interest (ROI) identified via Xenium Explorer.
...
roi_df <- read.csv("path/to/roi.csv") # cell stats from Xenium Explorer
...
Re-cluster the subsetted ROI data using both standard Seurat workflow and BANKSY.
...
Load the Xenium dataset for the Control (Ctrl) sample.

```{r}
raw_data_dir <- "path/to/raw_data" # Define directory
setwd(raw_data_dir)
NBB_Ctrl <- "sample_name" # File name
...
result_dir <- "path/to/results" # Define directory
setwd(result_dir)
```

### 1.3.2 Quality Control

Perform QC steps similar to the MS sample.
...
roi_df <- read.csv("path/to/roi_ctrl.csv") # cell stats from Xenium Explorer
roi_cells <- roi_df$Cell.ID
sc_roi <- subset(sc_final, cells = roi_cells)
...
Load the pre-processed ROI datasets for integration.
...
Integrate the MS and Control datasets to remove batch effects and enable comparative analysis.
...
Isolate the fibroblast clusters for further analysis.
...
Re-cluster the fibroblast subset.
...
Visualize the fibroblast clusters spatially and in UMAP space.
...
Find marker genes for the fibroblast subset.
...
Focus on dura and vascular fibroblasts.
...
Refine focus to only dura fibroblasts.
